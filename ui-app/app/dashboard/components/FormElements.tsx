...
export function FieldRenderer({
  field, stepData, onUpdate, aiMode, allData, onFieldFocus, activeFieldKey, readOnly,
}: FieldRendererProps) {
  ...
  const baseInputClass = `w-full min-w-0 px-3 sm:px-4 py-3 sm:py-3 bg-slate-50/50 border rounded-[16px] text-[14px] font-medium text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-600/5 transition-all duration-300 ${
    readOnly 
      ? 'bg-slate-100/50 border-slate-200 cursor-not-allowed text-slate-500'
      : isActive 
        ? 'ring-2 ring-indigo-400 border-indigo-400 bg-indigo-50/30 shadow-lg shadow-indigo-200/50' 
        : isAiFilled 
          ? 'border-indigo-200 bg-indigo-50/30' 
          : 'border-slate-200 focus:border-indigo-600 shadow-sm'
  }`;

  if (field.type === 'repeater_simple') {
    return <RepeaterSimple field={field} stepData={stepData} onUpdate={onUpdate} onFieldFocus={onFieldFocus} activeFieldKey={activeFieldKey} readOnly={readOnly} />;
  }
  if (field.type === 'repeater_pair') {
    return <RepeaterPair field={field} stepData={stepData} onUpdate={onUpdate} onFieldFocus={onFieldFocus} activeFieldKey={activeFieldKey} readOnly={readOnly} />;
  }
    ...
    <div className="space-y-2" data-field-key={field.key}>
      ...
      {field.type === 'textarea' ? (
        <textarea
          ...
          onChange={e => !readOnly && onUpdate(field.key, e.target.value)}
          onFocus={() => !readOnly && onFieldFocus?.(field.key)}
...
function RepeaterSimple({ field, stepData, onUpdate, onFieldFocus, activeFieldKey, readOnly }: {
  field: SchemaField;
  stepData: Record<string, unknown>;
  onUpdate: (key: string, value: unknown) => void;
  onFieldFocus?: (key: string) => void;
        ...
        {items.map((val, idx) => (
          <div key={idx} className="flex gap-3 animate-in fade-in slide-in-from-left-2 duration-300">
            <input
              ...
              onFocus={() => !readOnly && onFieldFocus?.(repActiveKey)}
              placeholder={field.placeholder || ''}
              readOnly={readOnly}
              className={`min-w-0 flex-1 px-3 sm:px-4 py-3 sm:py-3 bg-slate-50/50 border border-slate-200 rounded-[16px] text-[14px] font-medium text-slate-900 focus:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all shadow-sm ${
                readOnly 
...
function RepeaterPair({ field, stepData, onUpdate, onFieldFocus, activeFieldKey, readOnly }: {
  field: SchemaField;
  stepData: Record<string, unknown>;
  onUpdate: (key: string, value: unknown) => void;
  onFieldFocus?: (key: string) => void;
        ...
        {items.map((item, idx) => (
          <div key={idx} className="bg-gray-50/50 p-4 rounded-3xl border border-gray-100 flex flex-col gap-3 animate-in fade-in zoom-in-95 duration-300">
            ...
            <input
              ...
              onFocus={() => !readOnly && onFieldFocus?.(repKey)}
